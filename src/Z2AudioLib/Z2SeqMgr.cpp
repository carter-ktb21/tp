...
void Z2SeqMgr::subBgmStop() {
    switch (getSubBgmID()) {
        ...
        case Z2BGM_BOSS_SNOWWOMAN_D1:
            ...
            break;
        case Z2BGM_HIDDEN_VIL_D1:
            // Modded block to play added ast "zb-hidden_village.ast" (Not the intro)
            bgmStreamPrepare(0x2000085);
            bgmStreamPlay();
...
void Z2SeqMgr::bgmStreamPrepare(u32 bgmID) {
    if (mStreamBgmHandle) {
        if (bgmID == 0x2000089 && playDiababaAfterVulnerable) {
            bgmStreamStop(10);
        } else {
            bgmStreamStop(0);
        }
...
void Z2SeqMgr::fieldBgmFramework() {
        ...
        && mFlags.mFieldBgmPlay)
    {
        ...
        //     }
        // } else 
        if (getMainBgmID() == Z2BGM_FIELD_LINK_NIGHT) {
            if (!Z2GetStatusMgr()->isPaused()) {
